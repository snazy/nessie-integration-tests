...
for (sparkVersion in frameworksVersions["sparkVersions"].toString().split(",").map { it.trim() }) {
  // TODO version-gate for Spark 3.3 (not until Nessie 0.30.x)
  for (scalaVersion in
    frameworksVersions["sparkVersion-${sparkVersion}-scalaVersions"].toString().split(",").map {
      it.trim()
    }) {
    // The Iceberg build works only against one Scala version. So if the Iceberg build is inclued,
    // we have to restrict the integration tests to the same Scala version.
    if (!includeIcebergBuild || scalaVersion == System.getProperty("scalaVersion", "2.12")) {
      // TODO version-gate for Scala 2.13 (not until Nessie 0.30.x)
      includeProject(
        "nqeit-iceberg-spark-${sparkVersion}_$scalaVersion",
        file("nqeit-iceberg-spark")
      )
    }
  }
}

for (flinkVersion in frameworksVersions["flinkVersions"].toString().split(",").map { it.trim() }) {
  // TODO version-gate for Flink 0.15 (only since Iceberg 0.14.0)
  includeProject("nqeit-iceberg-flink-$flinkVersion", file("nqeit-iceberg-flink"))
}

for (prestoVersion in
  frameworksVersions["prestoVersions"].toString().split(",").map { it.trim() }) {
  // TODO version-gate for Presto (only Presto versions since 0.274 work with Nessie)
...
for (crossEngineSetup in
  ...
  val sparkMajor = frameworksVersions["crossEngine.$crossEngineSetup.sparkMajorVersion"]
  val scalaMajor = frameworksVersions["crossEngine.$crossEngineSetup.scalaMajorVersion"]
  val flinkMajor = frameworksVersions["crossEngine.$crossEngineSetup.flinkMajorVersion"]
  val prestoMajor = frameworksVersions["crossEngine.$crossEngineSetup.prestoMajorVersion"]
  includeProject(
    "nqeit-crosss-engine-$sparkMajor-$scalaMajor-$flinkMajor-$prestoMajor",
