...
System.setProperty("nessie.exclude-iceberg-builds", "true")
...
fun projectCanonicalPath(sub: String): File {
  ...
  val path = canonicalDir.path
  logger.lifecycle("Using '$property' from $path (via $projectDir)")
  return canonicalDir
}
...
includeBuild("$nessieSourceDir/iceberg") {
  name = "nessie-iceberg"
