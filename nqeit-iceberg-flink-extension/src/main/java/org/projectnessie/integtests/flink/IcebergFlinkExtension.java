...
public class IcebergFlinkExtension implements ParameterResolver {
  ...
  public Object resolveParameter(
      ...
      throws ParameterResolutionException {
    if (parameterContext.isAnnotated(Flink.class)) {
      FlinkPerTest flink = FlinkPerTest.get(extensionContext);
  ...
  private static class FlinkPerTest implements CloseableResource, FlinkHelper {
    private final FlinkHolder flinkHolder;
    private final String catalogName;
    private final String databaseName = "db";

    static FlinkPerTest get(ExtensionContext extensionContext) {
      return extensionContext
          .getStore(NAMESPACE)
          .getOrComputeIfAbsent(
              FlinkPerTest.class, c -> new FlinkPerTest(extensionContext), FlinkPerTest.class);
    }

    private FlinkPerTest(ExtensionContext extensionContext) {
      this.flinkHolder = FlinkHolder.get(extensionContext);

      catalogName =
          String.format(
              "%s_%s",
              extensionContext.getRequiredTestClass().getSimpleName().toLowerCase(Locale.ROOT),
              extensionContext.getRequiredTestMethod().getName().toLowerCase(Locale.ROOT));
